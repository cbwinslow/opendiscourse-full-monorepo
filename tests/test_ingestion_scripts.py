...
import opendiscourse.govinfo_scraper as gov_scraper
...
class TestCommitteeDownloader(unittest.TestCase):
    def test_get_all_committees(self):
        ...
        fake_response.json.return_value = {"committees": [{"id": "A1", "name": "Alpha"}]}
        fake_response.raise_for_status.return_value = None
        with patch.object(downloader.session, "get", return_value=fake_response) as mock_get:
            committees = downloader.get_all_committees()
            self.assertEqual(len(committees), 1)
            mock_get.assert_called_once_with(download_committee_data.COMMITTEE_BROWSE_URL)

class TestGovInfoScraper(unittest.TestCase):
    @patch("opendiscourse.govinfo_scraper.save_document")
    def test_fetch_metadata(self, mock_save):
        ...
        # doc summary
        r3 = MagicMock()
        r3.json.return_value = {"title": "Doc", "dateIssued": "2020-01-01", "dateLastModified": "2020-01-02"}
        ...
        with patch("requests.get", side_effect=responses):
            with patch("opendiscourse.govinfo_scraper.get_db_connection"):
