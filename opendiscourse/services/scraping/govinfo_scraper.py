...
def fetch_govinfo_metadata() -> None:
    ...
    for collection in collections.get("collections", []):
        ...
        try:
            ...
            logger.error("Failed to fetch documents for collection %s: %s", 
        ...
        for doc in documents.get("packages", []):
            try:
                ...
                try:
                    ...
                    logger.warning("No content available for document %s: %s", 
                                 doc_id, str(e))
                    content = ""
...
def save_document(title: str, content: str, metadata: dict) -> Optional[int]:
    ...
    try:
        ...
        if conn:
            conn.rollback()
        logger.error(
            "Unexpected error saving document: %s", 
            str(e),
            exc_info=True
        )
        return None
