...
from integration_constants import VERIFICATION_ENDPOINTS, WEBHOOK_TEST_PAYLOADS, SUCCESS_CODES, RETRYABLE_CODES
...
def verify_jira_connection(config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if response.status_code in SUCCESS_CODES:
            ...
            logging.error(f"Failed to verify Jira: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        logging.error(f"Error verifying Jira: {str(e)}")
        return False
...
def verify_github_connection(config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if response.status_code in SUCCESS_CODES:
            ...
            logging.warning(f"Temporary failure verifying GitHub: {response.status_code}")
            return False
        else:
            logging.error(f"Failed to verify GitHub: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        logging.error(f"Error verifying GitHub: {str(e)}")
        return False
...
def verify_bitbucket_connection(config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if response.status_code in SUCCESS_CODES:
            ...
            logging.warning(f"Temporary failure verifying Bitbucket: {response.status_code}")
            return False
        else:
            logging.error(f"Failed to verify Bitbucket: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        logging.error(f"Error verifying Bitbucket: {str(e)}")
        return False
...
def test_webhooks(config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if bitbucket_response.status_code not in SUCCESS_CODES:
            logging.error(f"Bitbucket webhook test failed: {bitbucket_response.status_code}")
            return False
        
        logging.info("All webhooks tested successfully")
        return True
    except Exception as e:
        logging.error(f"Error testing webhooks: {str(e)}")
        return False
...
def verify_repository_links(config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if response.status_code in SUCCESS_CODES and response.json():
            ...
            logging.warning(f"Temporary failure verifying repository links: {response.status_code}")
            return False
        else:
            logging.error(f"Failed to verify repository links: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        logging.error(f"Error verifying repository links: {str(e)}")
        return False
