...
def find_previous_version(doc_id: str, collection: str, conn: psycopg2.extensions.connection) -> Optional[Dict[str, Any]]:
...
def process_uslm(doc_id: str, content: str, collection: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        metadata = {
            ...
            'document_id': root.find('.//documentId').text if root.find('.//documentId') is not None else ''
        }
        
        # Validate schema
        schema_validation = validate_schema(content, collection)
        if not schema_validation['is_valid']:
            logging.error(f"Schema validation failed for {doc_id}: {schema_validation['errors']}")
            return None
            
        # Validate metadata
        metadata_validation = validate_metadata(metadata, collection)
        if not metadata_validation['is_valid']:
            logging.error(f"Metadata validation failed for {doc_id}: {metadata_validation['errors']}")
            return None
        ...
        if previous_version:
            ...
            if content_hash == previous_content_hash:
                logging.info(f"Document {doc_id} has not changed since version {previous_version['version']}")
                return None
        ...
        logging.error(f"Error processing USLM document {doc_id}: {str(e)}")
        return None
    finally:
        if 'conn' in locals():
            conn.close()
...
def process_cfr(doc_id, content):
    ...
    try:
        ...
        logging.error(f"Error processing CFR {doc_id}: {str(e)}")
        return None
...
def process_federal_register(doc_id, content):
    ...
    try:
        ...
        logging.error(f"Error processing Federal Register {doc_id}: {str(e)}")
        return None

def save_document(doc_id: str, content: str, metadata: Dict[str, Any], 
                                schema_validation: Dict[str, Any], metadata_validation: Dict[str, Any],
    ...
    try:
        ...
        """, (
            ...
            json.dumps(schema_validation.get('errors', []) + metadata_validation.get('errors', [])),
