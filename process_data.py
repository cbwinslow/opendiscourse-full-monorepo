...
class DataProcessor:
    ...
    def process_bill(self, xml_file):
        ...
        bill = {
            ...
            "subjects": [subj.text for subj in root.findall("subjects/legislativeSubjects/item")],
            "text_versions": self._process_text_versions(root.findall("textVersions/item")),
    ...
    def save_processed_data(self, collection_code, data):
        ...
        # Save as JSON
        json_file = os.path.join(collection_processed_dir, f"{collection_code}_{timestamp}.json")
        with open(json_file, 'w') as f:
            json.dump(data, f, indent=2)
        
        # Save as CSV (flatten nested structures)
        csv_file = os.path.join(collection_processed_dir, f"{collection_code}_{timestamp}.csv")
        df = pd.json_normalize(data)
        df.to_csv(csv_file, index=False)
        
        # Save as Parquet for efficient storage
        parquet_file = os.path.join(collection_processed_dir, f"{collection_code}_{timestamp}.parquet")
        df.to_parquet(parquet_file, index=False)
        
        print(f"Saved processed data for {collection_code} collection in multiple formats")
