...
from committee_api_config import COMMITTEE_DATA_DIR
...
class CommitteeDataProcessor:
    ...
    def process_committee_file(self, file_path):
        ...
        committee = {
            ...
            "subcommittees": self._process_subcommittees(data.get("committee", {}).get("subcommittees", [])),
            "members": self._process_members(data.get("committee", {}).get("members", [])),
    ...
    def save_processed_data(self, data):
        ...
        # Save as JSON
        json_file = os.path.join(self.processed_dir, f"committees_processed_{timestamp}.json")
        with open(json_file, 'w') as f:
            json.dump(data, f, indent=2)
        
        # Save as CSV (flatten nested structures)
        csv_file = os.path.join(self.processed_dir, f"committees_processed_{timestamp}.csv")
        df = pd.json_normalize(data)
        df.to_csv(csv_file, index=False)
        
        # Save as Parquet for efficient storage
        parquet_file = os.path.join(self.processed_dir, f"committees_processed_{timestamp}.parquet")
